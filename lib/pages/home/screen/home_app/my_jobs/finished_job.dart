...
class FinishJob extends StatefulWidget {
  ...
  const FinishJob({Key key, this.uid}) : super(key: key);
  @override
  _FinishJobState createState() => _FinishJobState(uid);
}

class _FinishJobState extends State<FinishJob> {
  final String uid;

  _FinishJobState(this.uid);
    ...
    return Scaffold(
      body: StreamBuilder(
          stream: FirebaseFirestore.instance
              .collection('posts')
              .where('userID', isEqualTo: uid).where('status',isEqualTo: 'done')
              .snapshots(),
        ...
        Row(
          ...
          children: [
            ...
            Container(
              ...
              child: Center(
                child: snapshot.data()['status'] == 'done'
                    ? Text('ສຳເລັດແລ້ວ')
                    : Text('ປິດ'),
              ),
            ),
