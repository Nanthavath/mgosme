...
class CurrentJob extends StatefulWidget {
  ...
  const CurrentJob({Key key, this.uid}) : super(key: key);

  @override
  _CurrentJobState createState() => _CurrentJobState(uid);
}

class _CurrentJobState extends State<CurrentJob> {
  final String uid;

  _CurrentJobState(this.uid);
    ...
    return Scaffold(
      body: StreamBuilder(
          stream: FirebaseFirestore.instance
              .collection('posts')
              .where('userID', isEqualTo: uid).where('status',isEqualTo: 'working')
              .snapshots(),
        ...
        Row(
          ...
          children: [
            ...
            Container(
              ...
              child: Center(
                child: snapshot.data()['status'] == 'working'
                    ? Text('ກຳລັງເຮັດ')
                    : Text('ປິດ'),
              ),
            ),
